...
export async function getStaticPaths() {
  ...
  return {
    ...
    // 수백 개의 게시물이 있는 블로그를 만들어도, 몇개는 아무도 보지않는 게시물이 있을수도 있다. 방문객이 없는 페이지의 사전생상은 시간과 자원 낭비다. 여기서 fallback: true이 필요하다. 그 후 사전에 생성할 페이지만 위에있는 paths에 남겨주면된다. 이러한 방식으로 방문율이 높은 페이지를 서버에 사전에 생성하고, 방문이 적은 페이지를 서버에서 생성하는것을 미루어 필요한 경우에만 사전 생성되게 할 수 있다. 정말 유용할 수 있다. 하지만 문제가 있다. 링크를 클릭하지않고, 직접 URL에 입력하게 되면 에러가 발생한다.
    fallback: true,
  };
}
